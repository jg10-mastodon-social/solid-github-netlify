...
- **History root** `GET /:page*/history` — unauthenticated. LDP `BasicContainer` listing the years from `REPO_START_YEAR` (resolved at build time via the order documented under [Build](#build)) through the current year. Zero GitHub API calls. `Cache-Control: public, max-age=86400, stale-while-revalidate=259200` (1 day fresh, 3 days SWR — the listing only changes at year boundaries).
