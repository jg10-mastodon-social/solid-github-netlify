...
- **History** — LDP-navigable view of past commits on `${GITHUB_REF}` affecting `<page>/*`. Full path pattern: `/:page*/history[/YYYY[/MM]]/<shortSha>[/<doc*>]`. Levels descend year → month → `<shortSha>/`, listing children at each step from `[REPO_START_YEAR, currentYear]`. Bucket levels are `Cache-Control: public, max-age=86400, stale-while-revalidate=259200`; years outside the range return 404, empty months return 200 with no children.
